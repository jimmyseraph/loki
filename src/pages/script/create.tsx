...
import LoopControllerForm, { initLoopControllerElement } from './form/loopController';
import IfControllerForm, { initIfControllerElement } from './form/ifController';
import VariableConfigForm, { initVariableConfigElement } from './form/variableConfig';
import { SCRIPT_NODE_RULE } from './form/common';
import { history } from 'umi';
import RegExpExtractorForm, { initRegExpExtractorElement } from './form/regexpExtractor';
import JSONPathExtractorForm, { initJSONPathExtractorElement } from './form/jsonpathExtractor';
import ResponseAssertionForm, { initResponseAssertionElement } from './form/responseAssertion';
...
const CreateOrUpdateScript: React.FC = (props: any) => {
    ...
    const [treeData, setTreeData] = useState<ScriptTreeDataNode[]>(initTreeData());
    ...
    useEffect(() => {
        ...
        if (pathname === "/script/edit") {
            Request({
                url: api.getScriptDetail.path,
                method: api.getScriptDetail.method,
                params: {
                    id: query?.id,
                }
            }, (res) => {
                let xmlStr: string = res.data.data.script;
                let xmlDoc = new DOMParser().parseFromString(xmlStr, "application/xml");
                setScriptXML(xmlDoc);
    ...
    const handleSave = () => {
        ...
        let script_name = scriptXML.getElementsByTagName("TestPlan")[0].getAttribute("label")
        Request({
            url: api.saveScript.path,
            method: api.saveScript.method,
            data: {
                id: query?.id,
                name: script_name,
    ...
    const handleMenuClick = (obj: MenuInfo, item: ScriptTreeDataNode, label: string) => {
        ...
        if (obj.keyPath[obj.keyPath.length - 1] === 'Threads') {  // Add thread
            console.log("add " + obj.key)
            setTreeData((origin) => updateTreeData(origin, item.key, children));
            let xmlNode: Element;
            if (obj.key === 'ThreadGroup') { // add ThreadGroup
        ...
        } else if (obj.keyPath[obj.keyPath.length - 1] === 'Controllers') { // add controller
            console.log("add " + obj.key)
            setTreeData((origin) => updateTreeData(origin, item.key, children));
            let xmlNode: Element;
            if (obj.key === 'LoopController') { // add LoopController
                xmlNode = initLoopControllerElement(children[0].key, children[0].title);
            }
            if (obj.key === 'IfController') { // add IfController
        ...
        } else if (obj.keyPath[obj.keyPath.length - 1] === 'Samplers') {  // add sampler
            console.log("add " + obj.key);
            setTreeData((origin) => updateTreeData(origin, item.key, children));
            let xmlNode: Element;
            if (obj.key === 'HelloSampler') { // add HelloSampler
                xmlNode = initHelloSamplerElement(children[0].key, children[0].title);
            }
            if (obj.key === 'HTTPSampler') { // add HTTPSampler
        ...
        } else if (obj.keyPath[obj.keyPath.length - 1] === 'Config') {  // add config
            console.log("add " + obj.key);
            setTreeData((origin) => updateTreeData(origin, item.key, children));
            let xmlNode: Element;
            if (obj.key === 'Variable') { // add Variable
        ...
        } else if (obj.keyPath[obj.keyPath.length - 1] === 'PostProcessor') {  // add PostProcessor
            console.log("add " + obj.key);
            setTreeData((origin) => updateTreeData(origin, item.key, children));
            let xmlNode: Element;
            if (obj.key === 'RegExpExtractor') { // add RegExp Extractor
                xmlNode = initRegExpExtractorElement(children[0].key, children[0].title);
            }
            if (obj.key === 'JSONPathExtractor') { // add JSON Path Extractor
                xmlNode = initJSONPathExtractorElement(children[0].key, children[0].title);
            }
            setScriptXML(origin => {
                let data = origin;
                data.getElementById(item.key as string)?.appendChild(xmlNode);
        ...
        } else if (obj.keyPath[obj.keyPath.length - 1] === 'Assertion') {  // add assertion
            console.log("add " + obj.key);
            setTreeData((origin) => updateTreeData(origin, item.key, children));
            let xmlNode: Element;
            if (obj.key === 'ResponseAssertion') { // add response assertion
                xmlNode = initResponseAssertionElement(children[0].key, children[0].title);
            }
            setScriptXML(origin => {
                let data = origin;
                data.getElementById(item.key as string)?.appendChild(xmlNode);
            ...
            setScriptXML((origin) => {
                let data = origin;
                data.getElementById(item.key as string)?.setAttribute("enabled", item.enabled ? "true" : "false");
                return data;
            });
    ...
    const handleSelect = (keys: React.Key[], info: {
        event: "select";
        selected: boolean;
        ...
        if (info.selected) {
            if (info.node.type === "TestPlan") {
                ...
                setCurrentForm(<PlanForm doc={scriptXML} xmlNode={element} onChange={handleChange} key={info.node.key} />);
            } else if (info.node.type === "ThreadGroup") {
                let element = scriptXML.getElementById(info.node.key as string);
                setCurrentForm(<ThreadGroupForm doc={scriptXML} xmlNode={element} onChange={handleChange} key={info.node.key} />);
            } else if (info.node.type === "HelloSampler") {
                let element = scriptXML.getElementById(info.node.key as string);
                setCurrentForm(<HelloSamplerForm doc={scriptXML} xmlNode={element} onChange={handleChange} key={info.node.key} />);
            } else if (info.node.type === "HTTPSampler") {
                let element = scriptXML.getElementById(info.node.key as string);
                setCurrentForm(<HTTPSamplerForm doc={scriptXML} xmlNode={element} onChange={handleChange} key={info.node.key} />);
            } else if (info.node.type === "LoopController") {
                let element = scriptXML.getElementById(info.node.key as string);
                setCurrentForm(<LoopControllerForm doc={scriptXML} xmlNode={element} onChange={handleChange} key={info.node.key} />);
            } else if (info.node.type === "IfController") {
                let element = scriptXML.getElementById(info.node.key as string);
                setCurrentForm(<IfControllerForm doc={scriptXML} xmlNode={element} onChange={handleChange} key={info.node.key} />);
            } else if (info.node.type === "Variable") {
                let element = scriptXML.getElementById(info.node.key as string);
                setCurrentForm(<VariableConfigForm doc={scriptXML} xmlNode={element} onChange={handleChange} key={info.node.key} />);
            } else if (info.node.type === "RegExpExtractor") {
                let element = scriptXML.getElementById(info.node.key as string);
                setCurrentForm(<RegExpExtractorForm doc={scriptXML} xmlNode={element} onChange={handleChange} key={info.node.key} />);
            } else if (info.node.type === "JSONPathExtractor") {
                let element = scriptXML.getElementById(info.node.key as string);
                setCurrentForm(<JSONPathExtractorForm doc={scriptXML} xmlNode={element} onChange={handleChange} key={info.node.key} />);
            } else if (info.node.type === "ResponseAssertion") {
                let element = scriptXML.getElementById(info.node.key as string);
                setCurrentForm(<ResponseAssertionForm doc={scriptXML} xmlNode={element} onChange={handleChange} key={info.node.key} />);
            } else {
    ...
    const loopTreeData = (
        ...
        key: React.Key,
        callback: (node: ScriptTreeDataNode, i: number, data: ScriptTreeDataNode[]) => void,
    ) => {
    ...
    const handleDrop: TreeProps['onDrop'] = info => {
        ...
        let dragCategory = dragElement.tagName === "TestPlan" ? "TestPlan" : dragElement.getAttribute("category") as string;
        if (dragCategory === "TestPlan") {
            return;
        }
        let dropCategory = dropElement.tagName === "TestPlan" ? "TestPlan" : dropElement.getAttribute("category") as string;
        if(!info.dropToGap) {
            ...
            if (dropPosition !== -1) {
                dropCategory = dropElement.parentElement?.tagName === "TestPlan" ? "TestPlan" : dropElement.parentElement?.getAttribute("category") as string;
                if(!Object(SCRIPT_NODE_RULE)[dropCategory].includes(dragCategory)) {
                    return;
                }
            }
    ...
    return (
        <>
            <Row gutter={24}>
                <Col>
                    <Space size={"large"}>
                        <Button type='primary' onClick={handleSave}>Save Plan</Button>
                ...
                <Col span={4}  >
                    ...
                    <Tree
                        ...
                        titleRender={(node): ReactNode => {
                            return (
                                <Dropdown overlay={<Menu items={dymMenu(node, handleMenuClick)} />} trigger={['contextMenu']}><div className={node.enabled ? 'enabled' : 'disabled'}>{node.title}</div></Dropdown>
                            )
                        }}
                    />
                </Col>
                <Col span={20}>
                    {selectKeys.length > 0 ? currentForm : <div></div>}
                </Col>
            </Row>
        </>
    )
...
const enableTreeNode = (list: ScriptTreeDataNode[], key: React.Key): ScriptTreeDataNode[] =>
    list.map(node => {
        if (node.key === key) {
...
const deleteTreeNode = (list: ScriptTreeDataNode[], key: React.Key): ScriptTreeDataNode[] => {
...
const updateTreeData = (list: ScriptTreeDataNode[], key: React.Key, children: ScriptTreeDataNode[]): ScriptTreeDataNode[] =>
    list.map(node => {
        if (node.key === key) {
            if (node.children) {
...
const dymMenu = (node: ScriptTreeDataNode, clickHandler: (info: MenuInfo, node: ScriptTreeDataNode, label: string) => void): ItemType[] => {
    return [
        // {
        //     key: "Threads",
        //     label: "Add Threads",
        ...
        // },
        {
            ...
            disabled: node && Object(SCRIPT_NODE_RULE)[node.category]? !Object(SCRIPT_NODE_RULE)[node.category].includes("Controller") : false,
            children: [
                {
                    key: "LoopController",
        ...
        },
        {
            key: "Samplers",
            label: "Add Samplers",
            disabled: node && Object(SCRIPT_NODE_RULE)[node.category]? !Object(SCRIPT_NODE_RULE)[node.category].includes("Sampler") : false,
            children: [
                {
                    key: "HTTPSampler",
                    label: "HTTP Sampler",
        ...
        },
        {
            key: "Config",
            label: "Add Config",
            disabled: node && Object(SCRIPT_NODE_RULE)[node.category]? !Object(SCRIPT_NODE_RULE)[node.category].includes("Config") : false,
            children: [
                {
                    key: "Variable",
                    label: "Variable",
        ...
        },
        {
            ...
            disabled: node && Object(SCRIPT_NODE_RULE)[node.category]? !Object(SCRIPT_NODE_RULE)[node.category].includes("PostProcessor") : false,
            children: [
                {
                    key: "RegExpExtractor",
        ...
        },
        {
            key: "Assertion",
            label: "Add Assertion",
            disabled: node && Object(SCRIPT_NODE_RULE)[node.category]? !Object(SCRIPT_NODE_RULE)[node.category].includes("Assertion") : false,
            children: [
                {
...
const parseTreeDataFromXML = (xml: Element): ScriptTreeDataNode[] => {
    ...
    let node: ScriptTreeDataNode = {
        type: xml.getAttribute("type") !== null ? xml.getAttribute("type") as string : "TestPlan",
        key: xml.getAttribute("id") !== null ? xml.getAttribute("id") as string : "",
        title: xml.getAttribute("label"),
        category: xml.getAttribute("category") !== null ? xml.getAttribute("category") as string : "TestPlan",
