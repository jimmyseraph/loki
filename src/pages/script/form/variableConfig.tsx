...
const VariableConfigForm: React.FC<ScriptFormProps> = (
  ...
  const handleFinish = () => {
    ...
    if (form.getFieldValue('vars') && form.getFieldValue('vars')[0]) {
      let children = (
        form.getFieldValue('vars') as { name: string; value: string }[]
      ).map((item) => generateParamElement(item.name, 'string', item.value));
    ...
    >
      ...
      <Row gutter={8}>
        <Col span={12}>
          <Form.List
            name="vars"
            initialValue={(getParamElements(xmlNode!) === null
              ? []
              : getParamElements(xmlNode!)
            ).map((item) => {
              return {
