...
import {
  ...
  generateParamElement,
  getParamElement,
  getParamElements,
  setParamArray,
  setParamValue,
  setParamValueAsCDATA,
} from './common';

export function initHTTPSamplerElement(key: string, label: string): Element {
  let node = generateComponentElement(
    key,
    label,
    'HTTPSampler',
    'Sampler',
  ...
  let node_follow_redirect = generateParamElement(
    'follow-redirect',
    'bool',
    'true',
  );
...
function initBody(xmlNode: Element | null) {
  let node_body = getParamElement(xmlNode!, 'body');
  if (xmlNode && node_body && node_body !== null) {
    return node_body.textContent as string;
  } else {
    return '';
  }
}
...
const HTTPSamplerForm: React.FC<ScriptFormProps> = (props: ScriptFormProps) => {
  ...
  const [body, setBody] = useState<string>(initBody(props.xmlNode));
  ...
  const handleFinish = () => {
    ...
    if (form.getFieldValue('headers') && form.getFieldValue('headers')[0]) {
      let headerStrArr = (
        form.getFieldValue('headers') as { name: string; value: string }[]
  ...
  return (
    ...
    >
      ...
      <Row gutter={8}>
        <Col span={4}>
          <Form.Item
            ...
            label="Protocol"
            initialValue={
              getParamElement(xmlNode!, 'protocol') === null
                ? 'HTTP'
                : (getParamElement(xmlNode!, 'protocol')?.innerHTML as string)
            }
          >
        ...
        <Col span={8}>
          <Form.Item
            ...
            initialValue={
              getParamElement(xmlNode!, 'domain') === null
                ? ''
                : (getParamElement(xmlNode!, 'domain')?.innerHTML as string)
            }
            rules={[{ required: true, message: 'Missing Server Name or IP' }]}
          >
        ...
        <Col span={3}>
          <Form.Item
            ...
            label="Port"
            initialValue={
              getParamElement(xmlNode!, 'port') === null
                ? 80
                : parseInt(
                    getParamElement(xmlNode!, 'port')?.innerHTML as string,
                  )
            }
          >
      ...
      <Row gutter={8}>
        <Col span={3}>
          <Form.Item
            name="method"
            initialValue={
              getParamElement(xmlNode!, 'method') === null
                ? 'GET'
                : (getParamElement(xmlNode!, 'method')?.innerHTML as string)
            }
          >
            <Select disabled={!enabled}>
        ...
        <Col span={8}>
          <Form.Item
            ...
            label="Path"
            initialValue={
              getParamElement(xmlNode!, 'path') === null
                ? ''
                : (getParamElement(xmlNode!, 'path')?.innerHTML as string)
            }
          >
            <Input disabled={!enabled} />
          </Form.Item>
      ...
      <Row gutter={8}>
        <Col>
          <Form.Item
            ...
            valuePropName="checked"
            initialValue={
              getParamElement(xmlNode!, 'follow-redirect') === null
                ? true
                : (getParamElement(xmlNode!, 'follow-redirect')
                    ?.innerHTML as string) === 'true'
            }
          >
        ...
        <Col>
          <Form.Item
            ...
            valuePropName="checked"
            initialValue={
              getParamElement(xmlNode!, 'ignore-tls') === null
                ? false
                : (getParamElement(xmlNode!, 'ignore-tls')
                    ?.innerHTML as string) === 'true'
            }
          >
        ...
        <Col>
          <Form.Item
            ...
            valuePropName="checked"
            initialValue={
              getParamElement(xmlNode!, 'enable-http2') === null
                ? false
                : (getParamElement(xmlNode!, 'enable-http2')
                    ?.innerHTML as string) === 'true'
            }
          >
      ...
      <Row gutter={8}>
        <Col span={10} data-color-mode="light">
          {/* <Form.Item
                        name="body"
